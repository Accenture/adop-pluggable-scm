...
class GitlabSCMProvider implements SCMProvider {
     ...
     *          Gitlab-HTTPS-with-context http://10.0.0.0:443/gitlab/myproject/
     *          Gitlab-HTTPS-without-context http://10.0.0.0:443/myproject/
     *
    ...
    void createScmRepos(String workspace, String repoNamespace, String codeReviewEnabled, String overwriteRepos) {
        ...
        // Create a group(if it does not exist) for the repos and fork each repository.
    ...
    /**
     * Checks that urls.txt exists, is readable non empty
     * @param urlsFile the file as an object
     * @param urlsFilePath the path to the file
     * @return true if urls.txt exists, is readable non empty and false otherwise
     */
    ...
    private void validate() {
        ...
        if (scmContext == null) {
            Logger.info("There is no context set for gitlab in adop-gitlab.props. If GitLab is behind a context such as /gitlab and http cloning is being configured in Load Cartridge cloning might not work")
            scmContext = ""
        }
    ...
    private String extractRepoNameFromUrl(String url) {
        ...
        result = url.split("/").findAll({ it.contains(".git") }).collect({
            it.replace(".git", "")
        }).get(0)
